...
void ConfigManager::loadBaseData()
{
	...
	Global::ConfigManagerObj->ModuleConfig.append(ke::AString("\tParsing type aliases ended.\n"));
...
void ConfigManager::loadMemoryStructures()
{
	...
	for (unsigned int i=0; i < files->length(); i++)
	{
		...
		time_t timestamp;
		memoryStructureNameToTimestamp.retrieve((char*)files->at(i).chars(), &timestamp);
